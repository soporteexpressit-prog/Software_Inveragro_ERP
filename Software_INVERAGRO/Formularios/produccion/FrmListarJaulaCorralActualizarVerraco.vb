...
Public Class FrmListarJaulaCorralActualizarVerraco
    Dim cn As New cnJaulaCorral
    Public idGalpon As Integer
    Public tipo As String
    Private ReadOnly _frmActualizarDatosVerraco As FrmActualizarDatosVerraco

    Public Sub New(frmActualizarDatosVerraco As FrmActualizarDatosVerraco)
        InitializeComponent()
        _frmActualizarDatosVerraco = frmActualizarDatosVerraco
    End Sub

    Private Sub FrmListarJaulaCorralActualizarVerraco_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub dtgListado_DoubleClickCell(sender As Object, e As Infragistics.Win.UltraWinGrid.DoubleClickCellEventArgs) Handles dtgListado.DoubleClickCell
        Try
            If (dtgListado.Rows.Count > 0) Then
                ...
                If activeRow IsNot Nothing AndAlso activeRow.Cells(0).Value.ToString().Length <> 0 Then
                    Dim codigo As String = activeRow.Cells("Codigo").Value.ToString()
                    Dim descripcion As String = activeRow.Cells("Descripcion").Value.ToString()
                    Dim sala As String = activeRow.Cells("Sala").Value.ToString()

                    _frmActualizarDatosVerraco.LlenarCamposJaulaCorral(codigo, descripcion, sala)
                    Me.Close()
                Else
